...
class MainActivity : FlutterActivity() {
    ...
    private val EXTRACTOR_CHANNEL = "newstube/extractor"
    private val REWRITER_CHANNEL = "newstube/rewriter"

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        try {
            NewPipe.init(OkHttpDownloader())
            Log.d("NewsTube", "NewPipe inicializado com sucesso")
        } catch (e: Throwable) {
            Log.e("NewsTube", "Erro ao inicializar NewPipe", e)
        }
    }
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        setupExtractorChannel(flutterEngine)
        setupRewriterChannel(flutterEngine)
    }

    private fun setupExtractorChannel(flutterEngine: FlutterEngine) {
        MethodChannel(
                ...
                "getCaptions" -> {
                    ...
                    Thread {
                        try {
                            ...
                            Log.e("NewsTube", "Erro ao obter captions", e)
                            runOnUiThread {
                                result.success(
                                    hashMapOf<String, Any?>(
                                        "hasCaptions" to false,
                    ...
                    Thread {
                        try {
                            ...
                            for (obj in items) {
                                ...
                                val looksLikeVideo =
                                    u.contains("watch", true) || u.contains("youtu.be", true)
                            ...
                            Log.e("NewsTube", "Erro em searchVideos", e)
                            runOnUiThread { result.error("SEARCH_FAIL", shortErr(e), null) }
                        }
                    }.start()
                }
                ...
                "searchChannels" -> {
                    ...
                    Thread {
                        try {
                            ...
                            Log.e("NewsTube", "Erro em searchChannels", e)
                            runOnUiThread { result.error("SEARCH_FAIL", shortErr(e), null) }
                        }
                    }.start()
                }
                ...
                "getChannelVideos" -> {
                    ...
                    Thread {
                        try {
                            ...
                            runOnUiThread { result.success(vids) }
                        } catch (e: Exception) {
                            Log.e("NewsTube", "Erro em getChannelVideos", e)
                            runOnUiThread { result.error("CHANNEL_FAIL", shortErr(e), null) }
                        }
                    }.start()
                }
    ...
    private fun setupRewriterChannel(flutterEngine: FlutterEngine) {
        val onDeviceRewriter = OnDeviceRewriter(applicationContext)
        ...
        MethodChannel(
            ...
            when (call.method) {
                "getRewriteAvailability", "getProofreadAvailability" -> {
                    val language = call.argument<String>("language") ?: "en"

                    Thread {
                        val availability = onDeviceRewriter.getRewriteAvailability(language)
                        runOnUiThread { result.success(availability) }
                    }.start()
                }

                "rewriteTranscript", "proofreadText" -> {
                    val text = call.argument<String>("text") ?: ""
                    val language = call.argument<String>("language") ?: "en"

                    Thread {
                        val rewritten = onDeviceRewriter.rewriteTranscript(text, language)
                        runOnUiThread { result.success(rewritten) }
                    }.start()
                }
    ...
    private fun findCaptionUrlByReflection(streamInfo: Any, preferLang: String): Pair<String, String>? {
        ...
        for (mName in containers) {
            try {
                val m = streamInfo.javaClass.methods.firstOrNull {
    ...
    private fun extractTracksFromContainer(container: Any): List<Any>? {
        ...
        for (n in names) {
            try {
                val m = container.javaClass.methods.firstOrNull {
                    it.name == n && it.parameterTypes.isEmpty()
                } ?: continue
    ...
    private fun channelVideosViaTabs(channelUrl: String, limit: Int): List<Map<String, Any?>> {
        ...
        val channelInfo = run {
            ...
            val m = ciClass.methods.firstOrNull {
                it.name == "getInfo" && it.parameterTypes.size == 2
        ...
        val tabs: List<Any> = run {
            val m = channelInfo.javaClass.methods.firstOrNull {
                it.name == "getTabs" && it.parameterTypes.isEmpty()
            }
    ...
    private fun tryGetTabExtractor(tab: Any): Any? {
        try {
            val m = tab.javaClass.methods.firstOrNull {
                it.name == "getContent" && it.parameterTypes.isEmpty()
            }
            val v = m?.invoke(tab)
            if (v != null) return v
        } catch (_: Throwable) {
        ...
        try {
            val m = tab.javaClass.methods.firstOrNull {
                it.name == "getTabExtractor" && it.parameterTypes.isEmpty()
            }
            val v = m?.invoke(tab)
            if (v != null) return v
        } catch (_: Throwable) {
    ...
    private fun doSearchItems(query: String): List<Any> {
        ...
        val info = run {
            val m = SearchInfo::class.java.methods.firstOrNull {
                it.name == "getInfo" && it.parameterTypes.size == 1
    ...
    private fun tryFetch(extractor: Any) {
        try {
            val m = extractor.javaClass.methods.firstOrNull {
                it.name == "fetchPage" && it.parameterTypes.isEmpty()
            }
            if (m != null) {
                m.invoke(extractor)
                return
        ...
        try {
            val m = extractor.javaClass.methods.firstOrNull {
                it.name == "fetch" && it.parameterTypes.isEmpty()
            }
            if (m != null) {
                m.invoke(extractor)
                return
    ...
    private fun tryCreateSearchExtractor(service: Any, query: String): Any? {
        try {
            val m = service.javaClass.methods.firstOrNull {
                it.name == "getSearchExtractor" && it.parameterTypes.size == 1
            }
        ...
        try {
            val m = service.javaClass.methods.firstOrNull {
                it.name == "searchExtractor" && it.parameterTypes.size == 1
            }
            if (m != null) return m.invoke(service, query)
        } catch (_: Throwable) {
        }
    ...
    private fun extractItemsList(info: SearchInfo): List<Any> {
        try {
            val m = info.javaClass.methods.firstOrNull {
                it.name == "getItems" && it.parameterTypes.isEmpty()
            }
            val v = m?.invoke(info)
        ...
        try {
            val m = info.javaClass.methods.firstOrNull {
                it.name == "getRelatedItems" && it.parameterTypes.isEmpty()
            }
            val v = m?.invoke(info)
            if (v is List<*>) return v.filterNotNull()
        } catch (_: Throwable) {
    ...
    private fun extractPublishedMillis(obj: Any): Long {
        ...
        for (mn in methodNames) {
            try {
                val m = obj.javaClass.methods.firstOrNull {
                    it.name == mn && it.parameterTypes.isEmpty()
                } ?: continue
